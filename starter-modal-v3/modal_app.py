...
app = modal.App("explaining-markets-v3")
...
image = (
    ...
    .pip_install(
        ...
        "scikit-learn",
        "numpy",
        # Gemini API for baseline prediction generation
        "google-generativeai",
    )
    .add_local_python_source("explaining_markets", "predict")
    .add_local_dir("model_v3", remote_path="/root/model_v3")
    .add_local_dir("model_artifacts", remote_path="/root/model_artifacts")
)
...
seen_webhooks = modal.Dict.from_name("em-webhook-dedupe-v3", create_if_missing=True)
...
def predict_and_submit(event: dict, webhook_id: str | None = None):
    ...
    print(f"[INFO] predict_and_submit called for event {event.get('event_id')}")
    print(f"[INFO] is_test={is_test(event)}")

    submitted = False
    try:
        # For TEST events, skip importing predict.py to save time
        if is_test(event):
            print(f"[DEBUG] TEST event - using neutral predictions")
            predictions = neutral_predictions(event)
        else:
            print(f"[DEBUG] Real event - loading model")
            from predict import predict
            predictions = predict(event)
        print(f"[INFO] Generated {len(predictions)} predictions: {predictions}")
        print(f"[INFO] Submitting for event_id: {event['event_id']}")

        result = submit_predictions(
            event_id=event["event_id"],
            predictions=predictions,
            config=Config.from_env(),
        )
        print(f"[INFO] Submission successful: {result}")
        submitted = True
    except Exception as exc:
        # Log loudly — `modal app logs explaining-markets-v3` finds it.
        print(f"[ERROR] prediction failed for event {event.get('event_id')}: {exc}")
        import traceback
        traceback.print_exc()
    finally:
        _release(webhook_id, submitted)
        print(f"[INFO] Released webhook {webhook_id}, submitted={submitted}")


@app.function(image=image, secrets=secrets)
@modal.asgi_app(label="explaining-markets-v3")
def web():
    ...
    @api.get("/")
    def health() -> dict:
        return {"ok": True, "service": "explaining-markets-v3"}
